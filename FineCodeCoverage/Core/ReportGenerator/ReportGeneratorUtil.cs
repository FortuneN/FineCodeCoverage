	...
	internal partial class ReportGeneratorUtil
	{
		...
		public static void ProcessCoberturaHtmlFile(string htmlFile, bool darkMode, out string coverageHtml)
		{
			coverageHtml = AssemblyUtil.RunInAssemblyResolvingContext(() =>
			{
				...
				var toParse = outerHtml.Substring(startIndex, endIndex - startIndex);
				endIndex = toParse.LastIndexOf(']');
				toParse = toParse.Substring(0, endIndex + 1);
				var parsed = JArray.Parse(toParse);
				foreach (JObject assembly in parsed)
				{
					...
					foreach (JObject @class in classes)
					{
						...
						else
						{
							...
							@class["rp"] = $"#{assemblyName}!{className + ".html" + rp.Substring(htmlIndex + 5)}";
						}
				...
				var replaced = parsed.ToString();
				htmlSb.Replace(toParse, replaced);
				...
				htmlSb.Replace("</body>", $@"
					<script type=""text/javascript"">
						...
						eventListener(document, 'click', function (event) {{
							...
							var fullHref = href.substring(1, htmlExtensionIndex + htmlExtension.length);
