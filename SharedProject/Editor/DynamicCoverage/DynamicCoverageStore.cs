...
namespace FineCodeCoverage.Editor.DynamicCoverage
{
    ...
    internal class DynamicCoverageStore : IDynamicCoverageStore
    {
        ...
        private class DynamicLine : IDynamicLine
        {
        ...
        public List<IDynamicLine> GetSerializedCoverage(string filePath)
        {
            var collectionExists = WritableUserSettingsStore.CollectionExists(dynamicCoverageStoreCollectionName);
        ...
        public void SaveSerializedCoverage(string filePath, IEnumerable<IDynamicLine> dynamicLines)
        {
            var serialized = JsonConvert.SerializeObject(dynamicLines.Select(dl => new DynamicLine
            {
                Number = dl.Number,
                CoverageType = dl.CoverageType
            }).ToList());
            var collectionExists = WritableUserSettingsStore.CollectionExists(dynamicCoverageStoreCollectionName);
            if (!collectionExists)
            {
