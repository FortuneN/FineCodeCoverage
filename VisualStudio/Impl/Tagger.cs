...
{
	public class Tagger<T> : ITagger<T> where T : ITag
	{
		...
		IEnumerable<ITagSpan<T>> ITagger<T>.GetTags(NormalizedSnapshotSpanCollection spans)
		{
			...
			foreach (var span in spans)
			{
				...
				var coverageLines = TestContainerDiscoverer.CoverageResult.GetLines(document.FilePath, startLineNumber, endLineNumber);
